...
class ClinicServiceTests {
	...
	void shouldFindAllPetTypes() {
		...
		PetType petType1 = petTypes.stream().filter(t -> t.getId() == 1).findFirst().orElseThrow();
		assertThat(petType1.getName()).isEqualTo("cat");
		PetType petType4 = petTypes.stream().filter(t -> t.getId() == 4).findFirst().orElseThrow();
	...
	void shouldInsertPetIntoDatabaseAndGenerateId() {
		...
		pet.setType(types.stream().filter(t -> t.getId() == 2).findFirst().orElseThrow());
